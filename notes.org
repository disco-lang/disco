...
* Next steps
  ...
  + Function properties/annotations
    + Idempotent, etc.
    + Memoization
    + Inverse
  ...
  + mechanism for language extensions (like GHC has)
  + Dynamic explorable error messages:
    - Dynamic witnesses for type errors (see ICFP 2016 paper,
      https://goo.gl/NCKXFi)

* Crazy/cool ideas

  + Type qualifiers like "nonzero", "nonnegative", "non-empty", etc.?
  + Type qualifiers for combinatorics (i.e. "reusable", "nonreusable", etc.?)
...
** Github issues
  ...
  Next steps that have been turned into Github issues:

  + (#8)  Chained comparisons   (2 < x <= 5)
  + (#11) Z/Q patterns (general arithmetic patterns?)
  + (#6)  polymorphism
  + (#12) Multi-clause definitions
  + (#13) QuickCheck properties in test suite
  + (#14) Finite types (like Fin n)
  + (#15) Stepwise execution (small step evaluation)
  + (#16) Graph reduction?  Don't re-evaluate...
...
* Nonlinear patterns
  ...
  Trickier than I thought.  The problem is that once we've done "bind"
  it's too late, because upon unbinding, the variables in the pattern
  will be chosen to be distinct.

  Basically we need to take care of this while parsing.
  - Add a new constructor for "back references" (in place of
    variables), which store some kind of de Bruijn index, or just an
    index counting unique variables in the pattern.

  - Keep track of which variable names we've seen while parsing a
    pattern.

  - When we see a name again, store it as a back reference.  These
    don't bind anything of course.

  When typechecking, need to similarly keep track of types of
  variables we've seen and their indices, and be able to look them up
  when encountering a "duplicate" variable.

  When desugaring, we have to replace duplicate markers with fresh
  names, and generate sets of variables that need to be tested for
  equality.
