...
-- Eager FCP + suspension (lazy) types.  See PFPL chapter 36.
...
lexer = T.makeTokenParser
  emptyDef { T.reservedNames = ["abort", "inl", "inr", "case", "unit", "fst", "snd", "fold", "unfold", "Void", "Unit", "Rec"] }
...
parseAtom
  =   EVar   <$> identifier
  <|> EAbort <$> (reserved "abort" *> parseExpr)
  <|> EInj
  <|> ECase  <$> (reserved "ifz" *> parseExpr)
