...
* Next steps
  ...
  + Documentation:
    - Freeform text (markdown?? See cheapskate library --- what Idris uses)
  ...
  + Guards
    - Merge if/when into a single syntax?
    - Use syntax "is" for pattern guards?  e.g.  when p is (x,y)
    - Then switch back to single = for equality testing?
