...
import           Disco.Typecheck.Constraints      (Constraint (CTrue))
...
data DiscoState = DiscoState
  ...
  , constraintLog :: IORef Constraint
...
newtype Disco a = Disco { unDisco :: DiscoM a }
  ...
  -- Type context reader (for tracking variable types during type checking)
  deriving (HasReader "tyctx" TyCtx, HasSource "tyctx" TyCtx) via
    (Rename "tyCtx"
    (Field "tyCtx" ()
    (MonadReader DiscoM)))

  -- Type definition reader (for tracking type definitions during type checking)
  ...
  -- Throwing & catching type checking errors
  deriving (HasThrow "tcerr" TCError, HasCatch "tcerr" TCError) via
    (SafeExceptions TCError DiscoM)

  -- Throwing & catching general errors
...
--   fails.
typecheckDisco
  :: forall (fwd :: [CC.Capability]) m a.
     (Has '[Ct "err", MonadIO] m, Has fwd m)
  => TyCtx
  -> TyDefCtx
  -> (forall m'. (Has '[Ct "tcerr", Rd "tyctx", Rd "tydefctx", Wr "constraints", Fresh, MonadIO] m', Has fwd m') => m' a)
  -> m a
typecheckDisco tyctx tydefs m
  = wrapError @"tcerr" @(Rename "TypeCheckErr" :.: Ctor "TypeCheckErr" "err") @(MonadIO : fwd) @TCError
  $ withLocalReader @_ @"tyctx" @TyCtx @(MonadIO : Ct "tcerr" : fwd)
    emptyCtx
  $ withLocalReader @_ @"tydefctx" @TyDefCtx @(MonadIO : Rd "tyctx" : Ct "tcerr" : fwd)
    M.empty
  $ fmap fst
  $ withLocalWriter @_ @"constraints" @Constraint @(MonadIO : Rd "tyctx" : Rd "tydefctx" : Ct "tcerr" : fwd)
  $ withTyDefns tydefs
  $ extends @"tyctx" tyctx
  m
...
loadDiscoModule :: Has '[Ct "err", MonadIO, Fresh] m => Resolver -> ModName -> m ModuleInfo
loadDiscoModule resolver m =
  fmap fst $
    withLocalState @_ @"modmap" @(M.Map ModName ModuleInfo) @'[Ct "err", Fresh] M.empty $
...
loadDiscoModule' ::
  Has '[Ct "err", MonadIO, Fresh, HasState "modmap" (M.Map ModName ModuleInfo)] m =>
...
loadDiscoModule' resolver inProcess modName  = do
  ...
  case M.lookup modName modMap of
    ...
    Nothing -> do
      ...
      m  <- typecheckDisco @'[Fresh] tyctx tydefns (checkModule cm)
