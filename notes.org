...
* Next steps
  ...
  + Use extensible tree type ("Trees that Grow")?
  + Use a single Disco monad for everything?
    - see blog post on "ReaderT pattern"?  Env data type and IORefs
      for state or something like that?

    Disco monad itself could have state for current definitions in
    scope; read-only env has things like language extensions enabled,
    various other options, etc.  Also want errors using Validation or
    something.  ... but that can't be a monad?  Not sure how that is
    supposed to work.

    Locally e.g. typechecking could use another ReaderT layer for the context?
  ...
  + Saturating subtraction ∸ for naturals and Q+
