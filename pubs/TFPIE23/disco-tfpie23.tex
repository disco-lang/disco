...
\todo{Internally, disco has qualified polymorphism, necessary to
  ...
  lam x. x - 2 has type a -> a for any type a which
...
foundation for mathematics.  In practice, mathematicians
usually behave more as if they were working in some kind of
type-theoretic foundation, which makes a statically typed functional
language a good match for mathematics as it is practiced (for example,
see \todo{cite HoTT and Lean}).  However, one area where there is a
big mismatch is in the distinction between \emph{types} and
\emph{sets}.
...
\emph{syntax} error.  Why the mismatch?  And what (if anything) can be
done about it?

In a typical discrete math course, sets are used for everything: in
addition to describing various finite, countably infinite, and
uncountably infinite collections of values, sets are used to describe
the domains and codomains of functions, or used as the domain of
discourse for quantifiers like $\forall$ and $\exists$.  \todo{Conflating
types and sets in a statically-typed functional language is
problematic:}

\todo{One way to think of types is as a particular collection of
  ``ur-sets'' with particularly nice structural properties.  For
  example, $\N$ is the unique set that includes $0$ and is closed
  under the successor operation; in contrast, there is nice
  structural way to define $\{2,4,7\}$ other than just listing its
  elements.  These nice structural properties is what enables
  decidable typechecking without having to resort to SMT solvers.}

\begin{itemize}
\item The ability to use arbitrary finite sets as types would lead to
  the need for tools like SMT solvers for typechecking, which would
  necessarily be incomplete.  For example, to typecheck %
  \verb|f : N -> {2,3,7}| would require somehow checking that for any
  natural number input, the function \verb|f| will always return a
  either $2$, $3$, or $7$.  This sort of \emph{refinement type} system
  ...
  non-starter.
\item The ability to use types as value-level sets introduces all
  sorts of difficulties, chief among which is the fact that most types
  correspond to \emph{infinite} sets.  Set values would have to be
  represented as some kind of syntax trees rather than simply as sets
  of values, and operations like membership checking become
  only \emph{semi}-decidable at best.
\end{itemize}

\todo{link to online discussions}
...
example, \mintinline{text}{forall x in [0..10]. x^2 <= 100}.  It
